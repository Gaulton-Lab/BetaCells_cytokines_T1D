...
def split_reads_and_call_peaks(args, c):
	...
	sort_cmd = ['sort', '-k1,1', '-k2,2n', '-S', '2G']
	with open(cluster_prefix + '.scale_1e6.bdg', 'w') as f:
		genomecov = subprocess.Popen(genomecov_cmd, stdout=subprocess.PIPE)
		subprocess.call(sort_cmd, stdin=genomecov.stdout, stdout=f)
	...
	os.remove(cluster_prefix + '_summits.bed')
	os.remove(cluster_prefix + '_peaks.xls')
	return
...
def find_marker_peaks(args):
	...
	for i in range(len(chunks)-1):
		...
		for cluster in sorted(set(clusters[args.cluster_col])):
			...
			z[cluster] = dmtx.apply(lambda x: sm.OLS(x, label_cov.values).fit().tvalues[1], axis=1)
			#z[cluster] = dmtx.apply(lambda x: sm.OLS(x, label_cov.values).fit().tvalues[1] if sum(x[dmtx.columns.isin(label_cov.loc[label_cov['OvR']==1].index)])>0 else np.nan, axis=1)
...
def process_args():
	...
	io_group.add_argument('-g', '--genome-file', required=False, default='/home/joshchiou/references/hg19.chrom.sizes', type=str, help='Genome sizes file')
